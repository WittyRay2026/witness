...
function createNeonSql(): Promise<Sql> {
  ...
  })().catch((err) => { globalRef.__pgSqlPromise__ = undefined; throw err; });
...
async function createPgliteSql(): Promise<Sql> {
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    return pg;
  })().catch((err) => { globalRef.__pgliteInstance__ = undefined; throw err; });
...
export async function getPglite(): Promise<import("@electric-sql/pglite").PGlite> {
  if (dbSource !== "pglite") throw new Error("getPglite() only without DATABASE_URL");
  await getSql();
  const pg = await globalRef.__pgliteInstance__;
  if (!pg) throw new Error("PGLite failed");
  return pg;
}
